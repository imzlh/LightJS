...
export default class Server {
    ...
    constructor(addr, debug = true) {
        this.#close_handler = bind(
            addr.includes('/') ? 'unix:' + addr : 'tcp://' + addr, 
            (client, addr) => {
    ...
    async handle(handler, addr) {
        ...
        console.log(handler.request.method, handler.request.path);
        ...
        if (handler.request.path.startsWith('/@debug/') && this.#debug){
            ...
            await handler.end;
            return handler.reuse().catch(e => console.error('failed to reuse keep-alive connection'));
        }
