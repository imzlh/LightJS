...
export default function serveFile(handler, file) {
    ...
    handler.headers.set('Content-Type', type);
    handler.headers.set('Content-Length', fs.mtime.toString());
    ...
    if(range){
        ...
        if(parts){
            ...
            size = end - start + 1;
            handler.headers.set('Content-Range', `bytes ${start}-${end}/${fs.size}`);
            handler.headers.set('Content-Length', size.toString());
            handler.status(206);
            try{
                fh.seek(start, 0);
            }catch{
    ...
    while(!fh.eof && fh.tell() < size){
        const chunk = fh.read(1024*1024);
        handler.send(chunk);
    }
