test('url', function(){
    ...
    assert(url.getQuery('query')[0] === 'string', 'url.getQuery("query") should be "string"');
    url.delQuery('query');
    assert(url.getQuery('query')[0] === undefined, 'url.getQuery("query") should be null after deleting it');
    url.addQuery('query', 'new_string');
    assert(url.getQuery('query')[0] === 'new_string', 'url.getQuery("query") should be "new_string" after adding it');
    assert(url.toString() === 'https://username:password@www.example.com:443/path/to/page.html?query=new_string#hash', 'url.toString() should be "https://username:password@www.example.com/path/to/page.html?query=new_string#hash"');
